...
describe("displayCalendarUserMessage", () => {
  ...
  it("keeps the original error for other failures", () => {
    expect(
      displayCalendarUserMessage("Failed to fetch calendar list", "calendar_list_failed"),
    ).toBe("Failed to fetch calendar list");
  });
});

describe("shouldShowGoogleCalendarErrorAlert", () => {
  it("hides token-expired alerts in display mode", () => {
    expect(shouldShowGoogleCalendarErrorAlert("Token expired. Please sign in again.", true)).toBe(
      false,
    );
  });
});
