...
function withCompletedRuntimes(
  state: TimersState,
  runtimes: TimerRuntime[],
  completed: TimerRuntime[],
): TimersState {
  ...
  return {
    ...
    pendingCompletions: [
      ...
      ...completed.filter((runtime) => !pendingIds.has(runtime.id)),
    ],
  };
}
...
function timersReducer(state: TimersState, action: TimersAction): TimersState {
  switch (action.type) {
    ...
    case "resume":
      return {
        ...
        runtimes: state.runtimes.map((runtime) =>
          runtime.id === action.id ? resumeRuntime(runtime, action.now) : runtime,
        ),
      };
    case "cancel":
      return {
    ...
    case "flush-completions": {
      ...
      return {
        ...state,
        pendingCompletions: state.pendingCompletions.filter((runtime) => !ids.has(runtime.id)),
      };
    }
  }
}
...
export function TimersProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    dispatch({
      type: "flush-completions",
      ids: state.pendingCompletions.map((runtime) => runtime.id),
    });
