...
describe("migrateDisplayDocument", () => {
  ...
  it("defaults missing household to an empty member list", () => {
    const doc = migrateDisplayDocument(v0Kitchen);
    expect(doc.household).toEqual({ members: [] });
  });

  it("keeps valid household members and drops invalid ones past 12", () => {
    const members = Array.from({ length: 14 }, (_, index) => ({
      id: `m${index}`,
      name: `Person ${index}`,
      color: `#00000${index.toString(16).padStart(1, "0")}`,
    }));
    members[1] = { id: "", name: "No id", color: "#fff" };
    members[2] = { id: "ok", name: "", color: "#fff" };
    const doc = migrateDisplayDocument({
      schemaVersion: 1,
      name: "Homeslate",
      views: [],
      ...
      settings: {},
      household: {
        members: [...members, { name: "missing-id" }, { id: "x", name: "Maya", color: "#6366f1" }],
      },
    });
    expect(doc.household?.members).toHaveLength(12);
    expect(doc.household?.members[0]).toEqual({
      id: "m0",
      name: "Person 0",
      color: "#000000",
    });
    expect(doc.household?.members.some((member) => member.id === "ok")).toBe(false);
    expect(doc.household?.members.at(-1)?.id).toBe("m13");
  });
