...
describe("FileDisplayStore", () => {
  ...
  it("put throws DisplayNotFoundError for an unknown id", async () => {
    const displays = await store();
    await expect(displays.put("missing", createEmptyDisplayDocument())).rejects.toBeInstanceOf(
  ...
  it("put rejects an escaping id without changing the external file", async () => {
    ...
    await expect(displays.put("../secret", createEmptyDisplayDocument("Changed"))).rejects.toThrow(
      "Invalid display id",
    );
